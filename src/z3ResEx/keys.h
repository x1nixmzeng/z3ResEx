/*
	z3ResEx
	Written by x1nixmzeng

	keys.h
		Extracted keys from client executables
	
	NOTES
		These keys are brute-forced
		The pragma regions allow sections to be collapsed
*/

#ifndef KEYS_H
#define KEYS_H

#pragma region Keys for early clients
const unsigned int Z3_KEY_LENGTH( 202 );

/*
	RaiderZ keys
		r32579	2F56493F59B78655FBCFEA894DE92D63	Perfect World Alpha
		r32732	A565DE84608B3FD16AAA2DB00EFFEA22	PMANG
		r34748	86FCEE748A4421FCBE0710EA1B235124	??
		r34991	F42E110E4203A24DB6BAE3B2E633570B	??
		r35358	0BC63ECE5B9C6BB56F959CF38943F920	Perfect World CBT
		r36163	EBFA48906C484DB5B412EB29D6BB20CA	Perfect World Stress Test
		r36249	BAB055DE137A9BCDD7F4E6D05D10BD0A	Gameforge OBT
		r36264	71F82E8D4DB80458CA00F8177897C6DF	Perfect World OBT
*/
unsigned char Z3_KEY_RAIDERZ_KR_NA[ Z3_KEY_LENGTH ] =
{
	 
};


/*
	RaiderZ keys
		r35337	8E405793A58A8A25EC45D1FFC0F860E1	Gameforge CBT
		r35608	60D16239B0DBC4E2F4E6774F69A5686E	PMANG Ep1
		r36373	32FD9047CE5A614AB130B6ED2BDAD0B9	PMANG
*/
unsigned char Z3_KEY_RAIDERZ_EU[ Z3_KEY_LENGTH ] =
{
	
};


/*
	GunZ2 Keys
		r13226	5F6D1AC14E5553317D231262356B7D32	Netmarble VIP/CBT
		r18687	D54651466D4AA086BA82AE807AFAC731	Netmarble CBT
		r25565	F1A7D95319DC217B3F5C326E94047953	Netmarble GunZ2 CBT2
		r30001	15A00851A1E9E0AAFAE44EDB105DDB22	Netmarble GunZ2 CBT (2013)
*/
unsigned char Z3_KEY_GUNZ2_KR[ Z3_KEY_LENGTH ] =
{
	 
};

#pragma endregion

const unsigned int Z3_KEY_LIST_LENGTH( 3 );

unsigned char *Z3_KEY_LIST[ Z3_KEY_LIST_LENGTH ] =
{
	Z3_KEY_RAIDERZ_KR_NA,
	Z3_KEY_RAIDERZ_EU,
	Z3_KEY_GUNZ2_KR
};

#pragma region Keys for current clients
const unsigned int Z3_KEY_LENGTH_METHOD2( 515 );

/*
	GunZ2 Keys (updated fileindex method)
		r33005		Feb 19th 2013
		r33297		Feb 25th 2013
*/

unsigned char Z3_KEY_GUNZ2_METHOD2[ Z3_KEY_LENGTH_METHOD2 ] =
{
     
};

/*
	RaiderZ Keys (updated fileindex method)
		r37874		Apr 2nd 2013

	NOTE
		Only last 64 bytes of key differs from GunZ2 key
*/

unsigned char Z3_KEY_RAIDERZ_METHOD2[ Z3_KEY_LENGTH_METHOD2 ] =
{
     
};
#pragma endregion

const unsigned int Z3_KEY_LIST_METHOD2_LENGTH( 2 );

unsigned char *Z3_KEY_LIST_METHOD2[ Z3_KEY_LIST_METHOD2_LENGTH ] =
{
	Z3_KEY_GUNZ2_METHOD2,
	Z3_KEY_RAIDERZ_METHOD2	// Only tested with EU client
};

#endif
